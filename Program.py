...
class TestApp(TestWrapper, TestClient):
    def __init__(self):
        ...
        self.low   = 10000
        self.close = 0
        self.highNArray = np.array([])
        self.lowNArray = np.array([])
        self.closeNArray = np.array([])
        self.willRCount = 0
        self.willR = 0
    ...
    def start(self):
        ...
        if self.globalCancelOnly:
            ...
            self.realTimeBarsOperations_req() #for williamsR
    ...
    def realTimeBarsOperations_req(self):
        ...
        #self.reqRealTimeBars(3001, ContractSamples.USStockAtSmart(), 5, "MIDPOINT", True, [])
        # ! [reqrealtimebars]
    ...
    def realtimeBar(self, reqId: TickerId, time:int, open_: float, high: float, low: float, close: float,
        ...
        self.highNArray = np.append(self.highNArray, high)
        self.lowNArray = np.append(self.lowNArray, low)
        self.closeNArray = np.append(self.closeNArray, close)
        if high > self.high:
                self.high = high
        if low < self.low:
                self.low = low
        if self.willRCount == 14: #12*5sec=1min  
                self.willR = talib.WILLR(self.highNArray, self.lowNArray, self.closeNArray, timeperiod=14)
                print("Williams R for ", self.willRCount, " period is ", self.willR)
                self.highNArray = np.array([])
                self.lowNArray = np.array([])
                self.closeNArray = np.array([])
                self.willRCount = 0
    ...
    def marketScannersOperations_req(self):
        ...
        #self.reqScannerSubscription(7001, ScannerSubscriptionSamples.MostActiveOpenInterest(), [], [])

        # Generic Filters
        tagvalues = []
        #tagvalues.append(TagValue("optVolumeAbove", "100"))
        #tagvalues.append(TagValue("avgVolumeAbove", "100"));
        #tagvalues.append(TagValue("abovePrice", "100"))
        tagvalues.append(TagValue("marketCapAbove", "500000000"))
        ...
        self.reqScannerSubscription(7003, scanSub, [], []) # requires TWS v973+
        #scanSub.belowPrice = UNSET_DOUBLE
        scanSub1 = ScannerSubscription()
        scanSub1.instrument = "STK"
        #scanSub1.locationCode = "STK.US.MAJOR"
        scanSub1.scanCode = "HOT_BY_PRICE_RANGE"
        scanSub1.id = "PRICE"
        scanSub1.abovePrice = 5000
        self.reqScannerSubscription(7004, scanSub1, [], []) # requires TWS v973+
        self.reqScannerSubscription(7005, scanSub, [], tagvalues) # requires TWS v973+
        
        scanSub2 = ScannerSubscription()
        scanSub2.instrument = "STK"
        #scanSub1.locationCode = "STK.US.MAJOR"
        scanSub2.scanCode = "HOT_BY_PRICE_RANGE"
        scanSub2.id = "PRICE"
        scanSub2.abovePrice = 5000
        self.reqScannerSubscription(7006, scanSub2, [], []) # requires TWS v973+
        scanSub3 = ScannerSubscription()
        scanSub3.instrument = "STK"
        scanSub3.locationCode = "STK.US.MAJOR"
        scanSub3.scanCode = "OPT_OPEN_INTEREST_MOST_ACTIVE"
        #scanSub3.id = "PRICE"
        #scanSub3.abovePrice = 5000
        self.reqScannerSubscription(7007, scanSub3, [], []) # requires TWS v973+
        
        #tagvalues1.append(TagValue("optVolumeAbove", "100"))
        #self.reqScannerSubscription(7004, ScannerSubscriptionSamples.PriceBelow(), [], []) # requires TWS v973+
        #self.reqScannerSubscription(7005, ScannerSubscriptionSamples.PriceAbove(), [], []) # requires TWS v973+
        #self.reqScannerSubscription(7006, ScannerSubscriptionSamples.MostActiveOpenInterest(), [], []) # requires TWS v973+
